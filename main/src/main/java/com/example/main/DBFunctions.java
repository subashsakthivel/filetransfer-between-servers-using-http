...
public class DBFunctions {
    ...
    public void insert_row(Connection conn, String table_name,String server_name, String file_name, String file_location, String time) {
        Statement statement;
        try {
            String query = String.format("insert into %s(server_name, file_name, file_location, time) values('%s','%s','%s','%s');",table_name,server_name, file_name, file_location, time);
            statement = conn.createStatement();
            statement.executeUpdate(query);
            System.out.println(file_name + " inserted to DB");
        }catch (Exception e) {
            System.out.println(e);
            e.printStackTrace();
        }
